...
BOOL LendRecInfoDlg::OnInitDialog()
{
	...
	CTime expTime(mLendRec.mDateInt + 60*24*3600);//设置可借阅时间
	CTime now = CTime::GetCurrentTime();//获取系统时间
	CString statusString;
	int temp = now.GetTime() - expTime.GetTime();//计算已借阅时间->计算是否达到罚款条件
	if(temp > 0){
		isExp = TRUE;
		int fine = temp / (24 * 3600);//罚款标准（一天一元）
		statusString.Format(_T(" (已过期，罚款 %d.%d 元)"), fine/10, fine%10);
	}else{
		statusString = _T(" (未过期)");
		isExp = FALSE;
	}
	SetDlgItemText(IDC_STATIC_LEND_STATUS,_T("还书日期：") + expTime.Format(_T("%Y-%m-%d %H:%M")) + statusString);//还书日期
	return TRUE;  
}
